...
from app.schemas.friendship import Friendship as FriendshipSchema

router = APIRouter()

@router.post("/{recipient_id}", response_model=FriendshipSchema, status_code=status.HTTP_201_CREATED)
def send_friend_request(recipient_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Send friend request to user"""
    requester = current_user.id
    
    # Check if recipient exists
    user_recipient = db.query(User).filter(User.id == recipient_id).first()
    if not user_recipient:
        raise HTTPException(status_code=404, detail="User not found")
    
    if requester == recipient_id:
        raise HTTPException(status_code=400, detail="Cannot add yourself as friend")
    
    # Determine low_user and high_user (sorted)
    low_user = min(requester, recipient_id)
    high_user = max(requester, recipient_id)
...
def update_friendship_status(friendship_id: int, status_str: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
